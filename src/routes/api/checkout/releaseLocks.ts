...
export const releaseLocks = async (uid: string, assetIDs: string[], releaseAll: boolean) => {
	return await store
...
export const post: RequestHandler = async (event) => {
	...
	const locksReleased = await releaseLocks(uid, assetIDs, releaseAll);

	return {
		body: { success: locksReleased }
	};
};
